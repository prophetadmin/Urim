...
Purpose
Write the canonical implementation-phase completion receipt for the resolved
Active Phase after direct proof is obtained for every non-receipt Exit
Criterion in the latest roadmap artifact.
...
Must write exactly one receipt for the resolved implementation phase.
Must record `Realization Mode` as one of `create`, `validate`, `extend`, or
`mixed`.
...
Must fail if the resolved phase is not `implementation`.
...
Resolved phase is not an implementation phase.
...
written for the resolved implementation phase after every non-receipt Exit
Criterion is proven directly.
